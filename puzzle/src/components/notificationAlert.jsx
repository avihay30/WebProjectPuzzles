...
import { is_response_ok, parse_response } from "../utils/utils"

export default class NotificationAlert extends Component {
  constructor() {
    super()
    this.state = { message: "", notifcation: null }
  }
  ...
  componentDidMount() {
    ...
    if (is_response_ok(response)) {
      this.setState({
        message: "The update or operation has been done successfully.",
        notifcation: this.successNotification,
      })
    } else if (response.status >= 300 && response.status < 500) {
      parse_response(response).then((responseMsg) => {
        this.setState({
          message: `${responseMsg}! Please change your data or try again later.`,
          notifcation: this.warningNotification,
        })
      })
    } else {
      parse_response(response).then((responseMsg) => {
        this.setState({
          message: `${responseMsg}! Please change your data or try again later.`,
          notifcation: this.dangerNotification,
        })
      })
    }
  }
  ...
  render() {
    const { message, notifcation } = this.state

    if (message && notifcation) {
      return (
        <div class="fixed top-6 rounded-lg z-50">{notifcation(message)}</div>
      )
    }
    return <></>
  }
